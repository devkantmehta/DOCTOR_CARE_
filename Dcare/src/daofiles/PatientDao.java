...
public class PatientDao {
	...
	try {	
			...
			PreparedStatement ps=conn.prepareStatement("select * from apointment where email=? and password=?");
	...
	public static int save(PatientBean pb)	{
		...
		try	{
			Connection conn=ConnectionProvider.getConnection();
			PreparedStatement ps=conn.prepareStatement("insert into apointment(id,name,dob,address,gender,contact,email,password) values(?,?,?,?,?,?,?,?)");
			
			ps.setInt(1,pb.getId());
	        ps.setString(2,pb.getName());
	        ps.setString(3,pb.getDob());
	        ps.setString(4,pb.getAddress());          
	        ps.setString(5,pb.getGender());  
	        ps.setString(6,pb.getContact());
	        ps.setString(7,pb.getEmail());
	        ps.setString(8,pb.getPassword());   
	 ...
	 public static List<PatientBean> getAllPatient(){  
	        ...
	        try{  
	            ...
	            PreparedStatement ps=con.prepareStatement("select * from apointment ORDER BY id");  
	            ResultSet rs=ps.executeQuery();  
	            while(rs.next()){  
	 ...
	 public static int update(PatientBean up){  
	        ...
	        try{  
	            ...
	            PreparedStatement ups=con.prepareStatement(  
	                         "update apointment set name=?,address=?,contact=?,password=? where email=?");  
